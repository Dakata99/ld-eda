...
def log_class_distribution(data: Table) -> None:
    ...
    if len(domain.class_vars) != 1:
        logger.warning(
            "Class distribution is shown only for single-target classification."
        )
        return
    ...
    if not class_var.is_discrete:
        logger.warning(
            "Class distribution is available only for discrete target variables."
        )
        return
...
def log_numeric_feature_statistics(data: Table) -> None:
    ...
    continuous_features = [
        (index, attr)
        for index, attr in enumerate(domain.attributes)
...
def load_orange_table(path: str | Path) -> Table:
    """
    Load an Orange .tab/.csv/.xlsx-supported file as an Orange Table.
    """
    path = Path(path)
    ...
    if not path.exists():
        logger.error("Dataset file does not exist: {}", path)
